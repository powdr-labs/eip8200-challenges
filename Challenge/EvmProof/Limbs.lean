...
theorem addThree256_value (x y z : UInt256) :
  ...
  all_goals simp [joinAt, addThreeAt, UInt256.toNat]
...
theorem fullMul256_value (a b : UInt256) :
  ...
  dsimp only at hhigh
  have hlowSplit : (mulSplit radix a.toNat b.toNat).1 =
      a.toNat * b.toNat % radix := rfl
  have hhighSplit : (mulSplit radix a.toNat b.toNat).2 =
      a.toNat * b.toNat / radix := rfl
  rw [hlowSplit, hhighSplit] at hhigh
  ...
  · simp only [if_pos hborrow]
    rw [if_neg (by omega)]
    have hhigh' :
        radix + a.toNat * b.toNat % (radix - 1) -
            a.toNat * b.toNat % radix - 1 =
          a.toNat * b.toNat / radix := by
