import Challenge.Bls12381G2Add.Reference.Proofs.SourceFpInvCall
import Challenge.EvmProof.ExecSound

set_option warningAsError true

/-! # Frozen G2ADD `fpInv` output and complete body -/
...
def fpInvHighEnv (yst : EvmState) (hi lo : U256) :
  ...
  VEnv.setMany (fpInvInitialEnv hi lo) ["\x0091"]
    [(fpInvResult yst hi lo).1]

def fpInvReturnEnv (yst : EvmState) (hi lo : U256) :
    VEnv Challenge.EvmProof.modexpExec.toDialect :=
  VEnv.setMany (fpInvHighEnv yst hi lo) ["\x0092"]
...
theorem fpInvReturnEnv_hi (yst : EvmState) (hi lo : U256) :
    (VEnv.get (fpInvReturnEnv yst hi lo) "\x0091").getD 0 =
      (fpInvResult yst hi lo).1 := by
  rfl

theorem fpInvReturnEnv_lo (yst : EvmState) (hi lo : U256) :
    (VEnv.get (fpInvReturnEnv yst hi lo) "\x0092").getD 0 =
...
theorem fpInvBodyResultEnv_hi (yst : EvmState) (hi lo : U256) :
    (VEnv.get (fpInvBodyResultEnv yst hi lo) "\x0091").getD 0 =
      (fpInvResult yst hi lo).1 := by
  rfl

theorem fpInvBodyResultEnv_lo (yst : EvmState) (hi lo : U256) :
    (VEnv.get (fpInvBodyResultEnv yst hi lo) "\x0092").getD 0 =
      (fpInvResult yst hi lo).2 := by
  rfl
