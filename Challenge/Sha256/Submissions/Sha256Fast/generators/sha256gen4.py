"""Optimized SHA-256 EVM bytecode, v4: loop-of-8 (proof-tractable).
...
KB = 0x980           # K[0..63], unpacked, parallel to W
MEM_END = 0x1180
...
class Gen:
    ...
    def dup_ptr(self):
        """Duplicate the group pointer, which lives directly below the eight
        working-variable slots."""
        self.a.dup(self.td + 9)
        self.td += 1
    ...
    def round(self, t):
        ...
        # K[8i+j] and W[8i+j] at q + (KB|WB) + 32j, q = 256i on the stack
        self.dup_ptr(); self.push(koff(t)); self.op("ADD", -1)
        self.op("MLOAD"); self.op("ADD", -1)
        self.dup_ptr(); self.push(woff(t)); self.op("ADD", -1)
        self.op("MLOAD"); self.op("ADD", -1)
    ...
    # --- schedule step, addressed off the group pointer u ---
    def dup_u(self):
        """The schedule-group pointer sits directly below the temporaries."""
        self.a.dup(self.td + 1)
        self.td += 1

    def load_rel(self, off):
        self.dup_u(); self.push(off); self.op("ADD", -1); self.op("MLOAD")

    def small_sigma_rel(self, off, shifts):
        self.load_rel(off)
        self.sigma_from_doubled(*shifts)

    def schedule_step_rel(self, j):
        """W[16+8g+j] from four earlier slots, all relative to u = 256g."""
        self.small_sigma_rel(woff(14 + j), (17, 19, 42))
        self.small_sigma_rel(woff(1 + j), (7, 18, 35))
        self.op("ADD", -1)
        self.load_rel(woff(9 + j)); self.op("ADD", -1)
        self.load_rel(woff(0 + j)); self.op("ADD", -1)
        self.push(MASK32); self.op("AND", -1)
        self.push(DBL); self.op("MUL", -1)
        self.dup_u(); self.push(woff(16 + j)); self.op("ADD", -1)
        self.op("MSTORE", -2)
    ...
    def body(self):
        a = self.a
        a.label("body")                                  # [ret]
        for j in range(16):                              # W[0..15] from SCR
        ...
        for j in range(8):
            p0 = a._pos
            self.schedule_step_rel(j)
            self.marks.append(("sched", j, p0, a._pos))
            assert self.td == 0, self.td
        a.push(256); a.op("ADD")
        a.push(6 * 256); a.dup(2); a.op("LT")
        a.push_label("sched"); a.op("JUMPI")
        a.op("POP")                                      # drop u
        a.push(0)                                        # q = 0, below state
        for i in range(7, -1, -1):                       # push h..a
        ...
        for j in range(8):
            p0 = a._pos
            self.round(j)
            self.marks.append(("round", j, p0, a._pos))
        assert self.slots == list("abcdefgh"), self.slots
        a.dup(9); a.push(256); a.op("ADD"); a.swap(9); a.op("POP")
        a.push(8 * 256); a.dup(10); a.op("LT")
        a.push_label("rounds"); a.op("JUMPI")
        for i in range(8):                               # H[i] += state
    ...
    def program(self):
        ...
        for i, k in enumerate(K):        # unpacked K table, parallel to W
            a.push(k); a.push(KB + 32 * i); a.op("MSTORE")
