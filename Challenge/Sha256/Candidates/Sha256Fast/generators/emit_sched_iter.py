...
""")
    name = "psched_iter" if with_ctrl else f"psched_probe{count}"
    ...
    thm = "sched_iter_continue" if with_ctrl else f"sched_probe{count}"
    if with_ctrl:
        end_pc = sched_dest
        end_stack = "(UInt256.ofNat 256 + u) :: rest"
...
"""
    ...
    out.append(f"""
/-- One iteration of the schedule loop{", control tail included (continue case)" if with_ctrl else f" truncated to {count} step(s) (probe)"}:
...
""")
    if with_ctrl:
        out.append(f"""  have hdest : Decode.isValidJumpDest bytes {sched_dest} = true := by
