import Challenge.Bls12381G2Msm.Reference.Proofs.SourceFp2AddDefs
import Challenge.Bls12381G2Msm.Reference.Proofs.SourceFpAdd

set_option warningAsError true

/-! The frozen scalar addition endpoint at the first Fp2 call's fuel. -/

namespace Challenge.Bls12381G2Msm.Reference.Proofs.SourceSemantics

open YulSemantics YulSemantics.EVM

theorem eval_fpAdd68 (ahi alo bhi blo : U256) (yst : EvmState) :
  ...
  have hstmt0 :
      Interp.execStmt Challenge.EvmProof.modexpExec 65 fpAddBodyFuns
        (fpAddInitialEnv ahi alo bhi blo) yst fpAddStmt0 =
      .ok (fpAddLowEnv ahi alo bhi blo, yst, .normal) := by
    rfl
  have hstmt1 :
      Interp.execStmt Challenge.EvmProof.modexpExec 64 fpAddBodyFuns
        (fpAddLowEnv ahi alo bhi blo) yst fpAddStmt1 =
      .ok (fpAddHighEnv ahi alo bhi blo, yst, .normal) := by
    rfl
  have hcondition :
      Interp.evalExpr Challenge.EvmProof.modexpExec 62 fpAddBodyFuns
    ...
    rfl
  have hbody :
      Interp.execStmt Challenge.EvmProof.modexpExec 67 fpAddFuns
        (fpAddInitialEnv ahi alo bhi blo) yst (.block fpAddBody) =
      .ok (fpAddFinalEnv ahi alo bhi blo, yst, .normal) := by
    have hseq : Interp.execStmts Challenge.EvmProof.modexpExec 66 fpAddBodyFuns
      ...
      · have hstmt2 :
            Interp.execStmt Challenge.EvmProof.modexpExec 63 fpAddBodyFuns
        ...
        have h2 := Interp.execStmts_cons_normal hstmt2
          (show Interp.execStmts Challenge.EvmProof.modexpExec 63 fpAddBodyFuns
            (fpAddHighEnv ahi alo bhi blo) yst [] =
    ...
    change (do
      let x ← Interp.execStmts Challenge.EvmProof.modexpExec 66 fpAddBodyFuns
        (fpAddInitialEnv ahi alo bhi blo) yst
        [fpAddStmt0, fpAddStmt1, fpAddStmt2]
  ...
  have hraw : Interp.evalExpr Challenge.EvmProof.modexpExec 68 fpAddFuns
      [("ahi", ahi), ("alo", alo), ("bhi", bhi), ("blo", blo)] yst
      (.call "\x004" [.var "ahi", .var "alo", .var "bhi", .var "blo"]) =
