import Challenge.Bls12381G2Msm.Reference.Proofs.SourceInvalidLengthPrefix
import Challenge.Bls12381G2Msm.SpecRefinement

set_option warningAsError true

/-! Executable size and invalid-branch lemmas for G2MSM. -/
...
private theorem lengthWord_mod_ne {yst : EvmState}
    ...
    (hrem : yst.env.calldata.length % Challenge.Bls12381G2Msm.pairBytes ≠ 0) :
    BitVec.ofNat 256 yst.env.calldata.length % BitVec.ofNat 256 288 ≠ 0 := by
  intro hzero
  ...
  norm_num at hnat
  exact hrem (by simpa [Challenge.Bls12381G2Msm.pairBytes] using hnat)
...
theorem exec_invalidLengthStmt {yst : EvmState} (funs V)
    ...
    Interp.execStmt modexpExec 222 funs (sizeEnv yst V) yst invalidLengthStmt =
...
theorem exec_sizeStmt (yst : EvmState) (funs V) :
    Interp.execStmt modexpExec 223 funs V yst sizeStmt =
