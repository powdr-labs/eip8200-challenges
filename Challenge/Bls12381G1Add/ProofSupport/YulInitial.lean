...
/-! # Concrete G1ADD source/target initial-state bridge -/
...
def sourceInitialState (code calldata : ByteArray) : EvmState :=
  let s := Challenge.Bls12381G1Add.initialState code calldata 0
  { EvmState.init with
    env := {
...
theorem sourceInitialState_matches (code calldata : ByteArray) (gas : Nat) :
    StateMatch (sourceInitialState code calldata)
      (Challenge.Bls12381G1Add.initialState code calldata gas) := by
  constructor
  · exact MemMatch.init
  · intro k
    exact conv_unconv _
  ...
  · constructor
    · simp [sourceInitialState, Challenge.Bls12381G1Add.initialState,
        YulEvmCompiler.ByteArray.toList_eq_data, Array.length_toList]
    · exact targetKeccakOracle_agrees
    all_goals simp [sourceInitialState, Challenge.Bls12381G1Add.initialState,
      conv_unconv]
  ...
  · constructor
    · intro a
      simpa [sourceInitialState, Challenge.Bls12381G1Add.initialState] using
        (Challenge.EvmProof.Bytes.memMatch_toList
          ((Challenge.Bls12381G1Add.initialState code calldata gas).accountMap
