...
/-!
# G1MSM specification adapter refinement

These compact equations isolate the strict wire decoder from later concrete
execution proofs.  Consumers can reason about one pair or the final adapter
...
theorem pairBytes_eq :
    pairBytes = Codec.g1Bytes + Codec.scalarBytes := rfl
...
private theorem packagedScalar_eq_some_iff (input : ByteArray) (offset : Nat)
    (point : EvmSemantics.Crypto.Bls12381.Point) (term : Msm.G1WireTerm) :
    (match Codec.decodeScalar input (offset + Codec.g1Bytes) with
      | none => none
      | some _ => some (point, scalarAt input (offset + Codec.g1Bytes))) =
        some term ↔
      point = term.1 ∧
        Codec.decodeScalar input (offset + Codec.g1Bytes) = some term.2.val := by
  split
  · rename_i hscalar
    constructor
    · intro h
  ...
  · rename_i scalar hscalar
    ...
    have hvalue : Codec.scalarWindowValue input (offset + Codec.g1Bytes) =
        scalar :=
      (Codec.decodeScalar_eq_some_iff input
        (offset + Codec.g1Bytes) scalar).1 hscalar |>.2
    constructor
    · intro hterm
...
theorem decodeTerm_eq_some_iff (input : ByteArray) (offset : Nat)
    ...
    decodeTerm input offset = some term ↔
      ...
      Codec.decodeScalar input (offset + Codec.g1Bytes) = some term.2.val := by
  change (Codec.decodeG1Subgroup input offset).bind (fun point =>
      match Codec.decodeScalar input (offset + Codec.g1Bytes) with
      | none => none
      | some _ => some (point, scalarAt input (offset + Codec.g1Bytes))) =
        some term ↔ _
...
theorem decodeTerm_subgroup {input : ByteArray} {offset : Nat}
    ...
    Subgroup.g1 term.1 = true := by
  have hpoint := (decodeTerm_eq_some_iff input offset term).1 hdecode |>.1
  exact (Codec.decodeG1Subgroup_eq_some_iff input offset term.1).1 hpoint |>.2
...
theorem spec_eq_some_iff {input output : ByteArray} :
    spec input = some output ↔
      input.size ≠ 0 ∧ input.size % pairBytes = 0 ∧
        ∃ terms, decodeTerms input 0 (input.size / pairBytes) = some terms ∧
          output = Codec.encodeG1 (Msm.g1Wire terms) := by
  unfold spec
  ...
  · have hnonempty : input.size ≠ 0 := fun h => hbad (Or.inl h)
    ...
    change (decodeTerms input 0 (input.size / pairBytes)).bind
        (fun terms => some (Codec.encodeG1 (Msm.g1Wire terms))) = some output ↔ _
    rw [Option.bind_eq_some_iff]
    constructor
...
theorem spec_success {input : ByteArray} {terms : List Msm.G1WireTerm}
    (hnonempty : input.size ≠ 0)
    (hmultiple : input.size % pairBytes = 0)
    (hterms : decodeTerms input 0 (input.size / pairBytes) = some terms) :
    spec input = some (Codec.encodeG1 (Msm.g1Wire terms)) := by
