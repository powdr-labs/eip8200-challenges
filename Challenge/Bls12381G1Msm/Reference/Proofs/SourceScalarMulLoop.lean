...
/-! Structural execution of the frozen 256-bit scalar loop from a point-add trace. -/
...
private theorem scalarMulStmt7_eq : scalarMulStmt7 =
...
theorem step_scalarMulFor (scalar point out : U256) {yst stend : EvmState}
    ...
    ExecStmt Challenge.EvmProof.modexpExec.toDialect scalarMulBodyFuns
      (scalarMulInitEnv scalar point out) yst scalarMulStmt7
      (scalarMulLoopEnv 0 scalar point out) stend .normal := by
  rw [scalarMulStmt7_eq]
  have hloop := step_scalarMulLoop htrace
  have hfor : ExecStmt Challenge.EvmProof.modexpExec.toDialect
      scalarMulBodyFuns (scalarMulInitEnv scalar point out) yst
      (.forLoop scalarMulLoopInit scalarMulLoopCondition
        scalarMulLoopPost scalarMulLoopBody)
      (restore (scalarMulInitEnv scalar point out)
        (scalarMulLoopEnv 0 scalar point out)) stend .normal := by
    apply Step.forLoop
    ...
    · rw [show hoist Challenge.EvmProof.modexpExec.toDialect
      ...
      simpa [scalarMulInitEnv, scalarMulLoopEnv] using hloop
  simpa [restore, scalarMulInitEnv, scalarMulLoopEnv,
    scalarMulInitialEnv] using hfor
