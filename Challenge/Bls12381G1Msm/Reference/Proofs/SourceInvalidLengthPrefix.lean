import Challenge.Bls12381G1Msm.Reference.Proofs.SourceInvalidLengthDefs
import Challenge.EvmProof.ModexpExec

set_option warningAsError true

/-! Interpreter prefix for the frozen G1MSM helper declarations. -/
...
theorem exec_reference_function_prefix (funs V st) :
    Interp.execStmts modexpExec 127 funs V st referenceBackendBlock =
      Interp.execStmts modexpExec 116 funs V st
        (sizeStmt :: invalidLengthStmt :: referenceBackendBlock.drop 13) := by
  have hprefix := execStmts_function_prefix modexpExec
    (referenceBackendBlock.take 11) (referenceBackendBlock.drop 11)
    reference_function_prefix 116 (by omega) funs V st
  rw [reference_function_prefix_length] at hprefix
  norm_num at hprefix
  calc
    Interp.execStmts modexpExec 127 funs V st referenceBackendBlock =
        Interp.execStmts modexpExec 127 funs V st
          (referenceBackendBlock.take 11 ++ referenceBackendBlock.drop 11) := by
      rw [List.take_append_drop]
    _ = Interp.execStmts modexpExec 116 funs V st
          (referenceBackendBlock.drop 11) := hprefix
    _ = Interp.execStmts modexpExec 116 funs V st
          (sizeStmt :: invalidLengthStmt :: referenceBackendBlock.drop 13) := by
...
theorem step_reference_function_prefix {funs V st V' st' outcome}
    (htail : ExecStmts modexpExec.toDialect funs V st
      ...
      V' st' outcome) :
    ExecStmts modexpExec.toDialect funs V st referenceBackendBlock
      V' st' outcome := by
  have htail' : ExecStmts modexpExec.toDialect funs V st
      (referenceBackendBlock.drop 11) V' st' outcome := by
    rw [reference_after_functions]
    exact htail
